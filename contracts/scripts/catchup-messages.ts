...
/**
 * Script pour rattraper TOUS les messages en retard
 * Scan les derniers blocs et traite les messages dans l'ordre des nonces
 */
    ...
    try {
        ...
        const gasPrice =
            destChain === 'polygon'
                ...
                : ethers.BigNumber.from(1000000) // Base: 0.001 Gwei
...
async function main() {
    console.log('🚀 Rattrapage des messages Base → Polygon (3 heures)')
    console.log('======================================================')
    ...
    console.log(`\n👤 Wallet: ${polygonSigner.address}`)

    // Scan les 3 dernières heures sur Base
    // Base: ~5 blocs/min × 180 min = 900 blocs
    console.log('\n⏳ Scan des 3 dernières heures sur Base...')
    const baseMessages = await scanMessages('base', baseProvider, 200)

    if (baseMessages.length === 0) {
        console.log('\n✅ Aucun message Base → Polygon trouvé dans les 3 dernières heures.')
        return
    }

    // Trier par nonce
    baseMessages.sort((a, b) => Number(a.nonce) - Number(b.nonce))

    console.log(`\n🔄 Traitement de ${baseMessages.length} message(s) Base → Polygon`)
    ...
    for (const msg of baseMessages) {
        const success = await processMessage(msg, 'polygon', polygonSigner)
        if (success) {
            successCount++
        } else {
            failCount++
